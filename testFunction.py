...
class operation:
    def __init__(self, cellIndex=1, cellType='offsta',time=9, fieldSize=7):
        self.fieldSize = fieldSize
        self.data = sio.loadmat('allFinal.mat')
        ...
        self.matrixSize =[self.patchNumber, int(2**time)]
        self.para = None
    ...
    def plotting(self, best_condition):
        ...
        print('time consume')
        print(now)
        X, Y = np.meshgrid(range(0, int(2 ** self.time)), self.patchSize)
        fig = plt.figure()
        ax1 = fig.add_subplot(121)
        ax1.contourf(X, Y, self.cell)
